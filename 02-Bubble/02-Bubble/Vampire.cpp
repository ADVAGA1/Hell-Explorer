...
void Vampire::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram) {
    ...
    sprite->setAnimationSpeed(TRANSFORM, 12);
    sprite->addKeyframe(TRANSFORM, glm::vec2(0.0f / 5.0f, 4.0f / 5.0f));
    sprite->addKeyframe(TRANSFORM, glm::vec2(1.0f / 5.0f, 4.0f / 5.0f));
    sprite->addKeyframe(TRANSFORM, glm::vec2(2.0f / 5.0f, 4.0f / 5.0f));
    sprite->addKeyframe(TRANSFORM, glm::vec2(3.0f / 5.0f, 4.0f / 5.0f));
...
void Vampire::update(int deltaTime) {
    ...
    if (currentState == CHANGE) {
        ...
        if(sprite->animation() != TRANSFORM)sprite->changeAnimation(TRANSFORM);
        ...
        if (!isBat) {
            ...
            if (isLanding)
            {
                ...
                if (nextTile == 2) {
                    isLanding = false;
                    isBat = false;
                    posEnemy.x -= SLIME_HITBOX_X - BAT_HITBOX_X;
                    ...
                    if (map->collisionMoveUp(glm::ivec2(posEnemy.x, posEnemy.y - 1), glm::ivec2(BAT_HITBOX_X, 1), false)) goUp = false;
                ...
                if (!goUp) {
                    ...
                    if (!map->collisionMoveUp(posEnemy, glm::ivec2(BAT_HITBOX_X, 1), false)) {
                        posEnemy.y -= 1;
                    }
                    else {
                        goUp = !goUp;
